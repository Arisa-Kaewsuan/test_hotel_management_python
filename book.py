...
class book():
    def __init__(self, command, status, age, name, room):
    ...
    def book_room(self):
        ...
        if (name_in != name_db) and (name_db is not None):
            ...
            sql = "UPDATE Hotel SET name_guest=%s,age_guest=%s,status=%s WHERE room_number=%s"
            val = (name_in, age, status, room_in)
            obj = db.connectDB(sql)
            obj.update_db(val)
            print(f"Room {room_in} is booked by {name_in} with keycard number keycard.")
